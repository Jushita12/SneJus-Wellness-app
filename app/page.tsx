...
export default function App() {
  ...
  useEffect(() => {
    const log = storage.getLog(activeUser, todayStr);
    if (log) {
      setTodayLog(log);
    } else {
    ...
    setFeed(storage.getFeed());
    setHistory(storage.getHistory(activeUser, 7));
  ...
  const updateLog = (updates: Partial<DailyLog>) => {
    ...
    setTodayLog(updated);
    storage.saveLog(activeUser, updated);
    console.log(`Updating log for ${activeUser}:`, updates);
  };
  ...
  const addMeal = (meal: Omit<Meal, 'id' | 'timestamp'>) => {
    ...
    console.log(`Adding meal for ${activeUser}: ${newMeal.description}`);
